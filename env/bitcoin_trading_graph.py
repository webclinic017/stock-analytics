...
class BitcoinTradingGraph:
    ...
    def _render_volume(self, current_step, net_worth, step_range, dates):
        ...
        pos = self.df['Price'].values[step_range] - \
            self.df['Price'].values[step_range] < 0
        neg = self.df['Price'].values[step_range] - \
            self.df['Price'].values[step_range] > 0
    ...
    def _render_trades(self, current_step, trades, step_range):
        for trade in trades:
            if trade['step'] in step_range:
                ...
                close = self.df['Price'].values[trade['step']]
                high = self.df['Price'].values[trade['step']]
                low = self.df['Price'].values[trade['step']]
